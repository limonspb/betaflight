...
void osdDrawSpec(displayPort_t *osdDisplayPort)
{
    if (!ARMING_FLAG(ARMED) && false)
    {
        ...
        const bool rpmLimitActive = mixerConfig()->rpm_limit == 1 && isProtocolBidirectionalDshot(&motorConfig()->dev);
        if (rpmLimitActive) {
        ...
        if (rpmLimitActive) {
            ...
            memset(buff,0,strlen(buff));
            len = tfp_sprintf(buff, "THR LIMIT %s", osdTableThrottleLimitType[currentControlRateProfile->throttle_limit_type]);
